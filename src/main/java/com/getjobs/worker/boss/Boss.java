...
public class Boss {
    ...
    public void prepare() {
        ...
        // 从统一黑名单表加载黑名单
        this.blackCompanies = platformBlacklistService.getBlackCompanies("boss");
        this.blackRecruiters = platformBlacklistService.getBlackRecruiters("boss");
        this.blackJobs = platformBlacklistService.getBlackJobs("boss");
    ...
    private void postJobByCity(String cityCode) {
        ...
        for (String keyword : config.getKeywords()) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                // 投递速度控制：每次投递后等待 18~22 秒（约每分钟3份）
                if (postCount > 0) {
                    int delayMs = 18000 + new Random().nextInt(4000);
                    log.info("投递间隔等待 {} 秒（模拟正常操作速度）...", delayMs / 1000);
                    PlaywrightUtil.sleepMillis(delayMs);
                }
