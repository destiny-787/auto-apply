...
public class PlaywrightManager {
  ...
  private static final String JOB51_URL = "https://www.51job.com";
     ...
     * 优先连接到系统 Edge 浏览器（避免反爬检测），其次连接已运行的 CDP 端口，
     * 最后才启动自带的 Chromium
     */
    public void init() {
        if (isInitialized()) {
        ...
        try {
            ...
            // ── 策略1：尝试连接已运行的 Edge（用户手动启动了调试端口）──
            String cdpUrl = "http://127.0.0.1:" + CDP_PORT;
            boolean connected = tryConnectOverCDP(cdpUrl);

            if (!connected) {
                // ── 策略2：自动启动系统 Edge 浏览器 ──
                log.info("未检测到 CDP 端口，正在启动你的系统 Edge 浏览器...");
                connected = tryLaunchSystemEdge();
                if (!connected) {
                    // ── 策略3：回退到自带 Chromium ──
                    log.warn("系统 Edge 启动失败，回退到自带 Chromium");
                    launchBundledChromium();
                }
            }

            log.info("✓ 浏览器已就绪 (调试端口: {})", CDP_PORT);

            // 创建共享的BrowserContext（使用系统浏览器的默认上下文）
            context = browser.newContext(new Browser.NewContextOptions()
                    .setViewportSize(null) // 不设置固定视口，使用浏览器窗口实际大小
                    .setUserAgent(
                            // 使用真实 Chrome UA，不加 Edge 标记 — Boss 直聘会检测异常的 UA
                            "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/135.0.0.0 Safari/537.36")
                    .setLocale("zh-CN")
                    .setTimezoneId("Asia/Shanghai")
                    .setColorScheme(null) // 使用系统默认
            ...
            // 打开配置管理前端页面
            configPage = context.newPage();
            configPage.setDefaultTimeout(DEFAULT_TIMEOUT);
            configPage.navigate("http://127.0.0.1:6866/ai-config");
            log.info("✓ 配置管理 Page已创建 (http://127.0.0.1:6866/ai-config)");
     ...
     * 尝试通过 CDP 连接到已运行的浏览器
     */
    private boolean tryConnectOverCDP(String cdpUrl) {
        try {
            browser = playwright.chromium().connectOverCDP(cdpUrl);
            log.info("✓ 已通过 CDP 连接到现有浏览器: {}", cdpUrl);
            return true;
        } catch (Exception e) {
            log.debug("CDP 连接失败 ({}): {}", cdpUrl, e.getMessage());
            return false;
        }
    }
     ...
     * 自动启动系统 Edge 浏览器并连接
     */
    private boolean tryLaunchSystemEdge() {
        try {
            ...
            pb.start();
            log.info("✓ 系统 Edge 浏览器已启动，等待 CDP 就绪...");

            // 等待 CDP 端口就绪（最多 15 秒）
            for (int i = 0; i < 30; i++) {
                try { Thread.sleep(500); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); break; }
                if (tryConnectOverCDP("http://127.0.0.1:" + CDP_PORT)) {
                    return true;
                }
            }
            log.warn("Edge 已启动但 CDP 连接超时");
            return false;
        } catch (Exception e) {
            log.warn("启动系统 Edge 失败: {}", e.getMessage());
            return false;
        }
    }

    /**
     * 启动自定义 Chrome/Chromium（回退方案）
     * 使用本地安装的 Chrome，并添加反检测启动参数
     */
    private void launchBundledChromium() {
        browser = playwright.chromium().launch(new BrowserType.LaunchOptions()
                .setHeadless(false)
                .setSlowMo(50)
                .setExecutablePath(Paths.get("D:\\chromium-test\\chrome-win\\chrome.exe"))
                .setArgs(List.of(
                        "--remote-debugging-port=" + CDP_PORT,
                        "--start-maximized",
                        // 关键反检测参数
        ...
        log.info("✓ 自定义 Chrome 已启动 (带反检测参数)");
    }

    /**
     * 在上下文层统一注入 Boss 脚本，对所有 zhipin.com 子域名生效。
     */
    private void injectBossInitScript(BrowserContext targetContext) {
        ...
        // 对 zhipin.com 的任意子域名（www, login, passport 等）都注入
        String wrapped = "(function(){try{if(location&&location.hostname&&location.hostname.endsWith(\"zhipin.com\")){"
