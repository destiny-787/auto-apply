...
public class Zhilian {
    ...
    private final PlatformBlacklistService platformBlacklistService;
    ...
    private boolean deliverCurrentPage(String keyword) {
        try {
            ...
            // 加载黑名单
            java.util.Set<String> blackCompanies = platformBlacklistService.getBlackCompanies("zhilian");
            java.util.Set<String> blackJobs = platformBlacklistService.getBlackJobs("zhilian");
            java.util.Set<String> blackKeywords = platformBlacklistService.getBlackKeywords("zhilian");
            ...
            for (PageJob pj : jobs) {
                ...
                // 黑名单过滤
                if (pj.jobTitle != null && blackJobs != null && blackJobs.stream().anyMatch(pj.jobTitle::contains)) {
                    log.info("智联黑名单过滤-职位: {} | 公司: {}", pj.jobTitle, pj.companyName);
                    continue;
                }
                if (pj.companyName != null && blackCompanies != null && blackCompanies.stream().anyMatch(pj.companyName::contains)) {
                    log.info("智联黑名单过滤-公司: {} | 职位: {}", pj.companyName, pj.jobTitle);
                    continue;
                }
                if (pj.jobTitle != null && blackKeywords != null && blackKeywords.stream().anyMatch(pj.jobTitle::contains)) {
                    log.info("智联黑名单过滤-关键词命中职位: {} | 公司: {}", pj.jobTitle, pj.companyName);
                    continue;
                }
