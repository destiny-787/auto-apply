...
public class Job51 {
    ...
    private void deliverCurrentPage() {
        try {
            ...
            // 加载黑名单
            java.util.Set<String> blackCompanies = platformBlacklistService.getBlackCompanies("job51");
            java.util.Set<String> blackJobs = platformBlacklistService.getBlackJobs("job51");
            java.util.Set<String> blackKeywords = platformBlacklistService.getBlackKeywords("job51");
            ...
            for (int i = 0; i < jobCount; i++) {
                ...
                try {
                    ...
                    // 黑名单过滤
                    if (title != null && blackJobs != null && blackJobs.stream().anyMatch(title::contains)) {
                        log.info("51job黑名单过滤-职位: {} | 公司: {}", title, company);
                        continue;
                    }
                    if (company != null && blackCompanies != null && blackCompanies.stream().anyMatch(company::contains)) {
                        log.info("51job黑名单过滤-公司: {} | 职位: {}", company, title);
                        continue;
                    }
                    if (title != null && blackKeywords != null && blackKeywords.stream().anyMatch(title::contains)) {
                        log.info("51job黑名单过滤-关键词命中职位: {} | 公司: {}", title, company);
                        continue;
                    }
